...
The output CSVs are for reference. The hub (Col 2) works directly from the mask TIFs generated by Col 1 — no CSV import required.
...
| File | Content |
|------|---------|
| `<output_dir>/masks/` | Sparkle masks (uint16 TIF, one per image) |
| `<output_dir>/cell_masks/` | Cell masks (uint16 TIF, one per image) |
